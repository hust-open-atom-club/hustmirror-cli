...
_cran_install_1() {
	...
	if [ -f ~/.Rprofile ]; then
		...
		[ -f ${_backup_dir}/cran___.Rprofile.bak ] || cp ~/.Rprofile ${_backup_dir}/cran___.Rprofile.bak || {
...
uninstall() {
	...
	if [ -f ${_backup_dir}/cran___.Rprofile.bak ]; then
		set_sudo
		cp "${_backup_dir}/cran___.Rprofile.bak" ~/.Rprofile 2>/dev/null || true
...
can_recover() {
	...
	[ -f ${_backup_dir}/cran___.Rprofile.bak ]
}

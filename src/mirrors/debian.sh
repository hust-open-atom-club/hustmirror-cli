...
uninstall() {
	...
	if [ -f ${_backup_dir}/debian__etc_apt_sources.list.d_ubuntu.sources.bak ]; then
		set_sudo
		cp "${_backup_dir}/debian__etc_apt_sources.list.d_ubuntu.sources.bak" /etc/apt/sources.list.d/ubuntu.sources 2>/dev/null || true
		print_info "Restored /etc/apt/sources.list.d/ubuntu.sources"
	fi
	if [ -f ${_backup_dir}/debian__etc_apt_sources.list.bak ]; then
		set_sudo
		cp "${_backup_dir}/debian__etc_apt_sources.list.bak" /etc/apt/sources.list 2>/dev/null || true

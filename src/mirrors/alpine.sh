...
install() {
	...
	_alpine_linux_install_1 || return 1
	_alpine_linux_install_2 || return 1
...
uninstall() {
	...
	if [ -f ${_backup_dir}/alpine-linux__etc_apk_repositories.bak ]; then
		set_sudo
		cp "${_backup_dir}/alpine-linux__etc_apk_repositories.bak" /etc/apk/repositories 2>/dev/null || true

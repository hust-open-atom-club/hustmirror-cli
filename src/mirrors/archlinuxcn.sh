...
_arch_linux_cn_install_1() {
	...
	if [ -f /etc/pacman.conf ]; then
		...
		[ -f ${_backup_dir}/arch-linux-cn__etc_pacman.conf.bak ] || $sudo cp /etc/pacman.conf ${_backup_dir}/arch-linux-cn__etc_pacman.conf.bak || {
			print_error "Backup /etc/pacman.conf failed"
			return 1
		}
		$sudo  sed -i -E -e "1i\\[archlinuxcn]\nServer = ${_http}://${_domain}/archlinuxcn/\$arch" /etc/pacman.conf || {
...
install() {
	...
	_arch_linux_cn_install_1 || return 1
	_arch_linux_cn_install_2 || return 1
	_arch_linux_cn_install_3 || return 1
...
uninstall() {
	...
	if [ -f ${_backup_dir}/arch-linux-cn__etc_pacman.conf.bak ]; then
		set_sudo
		cp "${_backup_dir}/arch-linux-cn__etc_pacman.conf.bak" /etc/pacman.conf 2>/dev/null || true
...
can_recover() {
	...
	[ -f ${_backup_dir}/arch-linux-cn__etc_pacman.conf.bak ]
}
